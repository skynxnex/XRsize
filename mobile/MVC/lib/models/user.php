...
class UserModel {
	...
	function create($info,$login = true){
		...
		$password = sha1(mysql_real_escape_string($info['password']) . PASSWORD_SALT);
		...
		$db->query("SELECT id, pass FROM user WHERE email = '".mysql_real_escape_string($email)."'");
		if(mysql_num_rows($db->result) == 1){
			$this->msg = "Error! E-mail address is already in use.";
		}else{
			$query = $db->query("INSERT INTO user (name,email,pass,status,created_ip) VALUES ('$name','$email','$password','$status','$created_ip')");
			if($query){
				$this->msg = "User successfully added.";
				$this->ok = true;
				if($login) $this->login($info['email'],$info['pass']);
				return true;
			}else{
	...
	function update($info){
		...
		$query = "UPDATE user SET ".$sql." WHERE id = '".$this->user_id."'";
	...
	function login($email, $password){
		global $db;
		$sql = $db->query("SELECT * FROM user WHERE user_name = '".mysql_real_escape_string($email)."'");
		$this->ok = false;
		if(!$email || !$password){
		...
		if($db->num($sql) == 1)
		{
			$db_password = $results['pass'];
			$name = $results['name'];
			if(sha1($password . PASSWORD_SALT) == $db_password) {
				$_SESSION['auth_email'] = $email;
				$_SESSION['auth_secret'] = sha1($password . PASSWORD_SALT);
				setcookie("auth_email", $email, time()+60*60*24*30, "/", COOKIE_DOMAIN);
				setcookie("auth_secret", sha1($password . PASSWORD_SALT), time()+60*60*24*30, "/", COOKIE_DOMAIN);
	...
	function check($email, $secret){
		global $db;
		$sql = $db->query("SELECT id, pass, name FROM user WHERE user_name = '".mysql_real_escape_string($email)."'");
		$results = $db->fetch($sql);
		if($db->num($sql) == 1)
		{
			$db_password = $results['pass'];
	...
	function get_info($field = "*", $email = null){
		...
		$sql = $db->query("SELECT $field FROM user WHERE email = '$email'");
		$info = $db->fetch($sql);
	...
	function user_table($all = false, $start = 1, $limit = 20, $sort_by = "id DESC"){
		global $user, $db;
		$query = "SELECT * FROM user";
		if(!$all) $query .= " WHERE id = '".$user->user_id."'";
		...
		if($this->total >= 1){
			...
			while($row = $db->fetch($result)){
				...
				if($all) $str .= '<a href="admin/users/edit/'.$row['id'].'">'.$this->status($row['status']).'</a>';
	...
	function user_update($info, $user_id){
		global $db;
		$query = "UPDATE user SET status='".$info['status']."', name='".$info['name']."', email='".$info['email']."', admin='".$info['admin']."'";
		if($info['password']){
			$password = sha1(mysql_real_escape_string($info['password']) . PASSWORD_SALT);
			$query .= ", pass='".$password."'";
		}
